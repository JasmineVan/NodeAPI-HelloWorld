...
function create(options) {
		...
		(function next(err) {
			if (err) {
				...
				case 0: /* Response to the client. */
					...
					headers['Content-Length'] = res.__.length;
	...
	/* Make the sweet chaining API. */
	
	var pipelines = {},
		current = null;
	
	// Create a new pipeline.
	listener.mount = function (point) {
		if (!(current = pipelines[point])) {
			// Switch current pipeline to the new created.
			current = pipelines[point] = router.mount(point);
		}
		return listener;
	};
	
	// Use a filter on the current pipeline.
	listener.use = function (filter) {
		if (current) {
			current.use(filter);
		}
		return listener;
